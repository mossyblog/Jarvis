...
public class PermissionService : IPermissionService
{
    ...
    public async Task InvalidateCacheForRoleAsync(Guid roleId)
    {
        ...
        var profiles = await _dataContext.Query()
            .WithAll<SecurityProfile>(Filter<SecurityProfile>.All())
            .ToEntityComponents();
    ...
    public async Task<HashSet<string>> GetUserPermissionsAsync(Guid userId)
    {
        ...
        // Get permissions from all roles (per jarvis-15 ADR - no denormalized PermissionIds)
    ...
    private async Task<Permission?> GetPermissionById(Guid permissionId)
    {
        var permissions = await _dataContext.Query()
            .WithAll<Permission>(Filter<Permission>.Eq(p => p.OwnerEntityId, permissionId))
            .ToEntityComponents();
